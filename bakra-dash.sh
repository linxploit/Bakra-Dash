...
cloudflare_tunnel() {
    ...
    # Replace the forwarding link in template and copy to index.html
    sed "s+forwarding_link+$link+g" "$TEMPLATE_FILE" > index.html
    checkfound
}
...
localhost_server() {
    ...
    sed "s+forwarding_link+http://localhost:3333+g" "$TEMPLATE_FILE" > index.html
    checkfound
}
